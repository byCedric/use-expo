...
// see: https://github.com/react-navigation/navigation-ex/blob/324fad33ef1ee3f9f960c68a7f8ea74ba17b377b/example/metro.config.js
...
const modules = ['@expo/vector-icons']
	...
	.filter(
		...
		(m, i, self) => self.lastIndexOf(m) === i && !m.startsWith('@use-expo/')
	);
	...
	resolver: {
		...
		blacklistRE: blacklist(
			fs
				...
				.map(it => new RegExp(`^${escape(path.join(it, 'node_modules'))}\\/.*$`))
		),
			...
			return (req, res, next) => {
				...
				if (req.url.startsWith(assets)) {
					...
					req.url = req.url.replace(
						assets,
						'/assets/../node_modules/react-navigation-stack/lib/module/views/assets',
					);
				}
